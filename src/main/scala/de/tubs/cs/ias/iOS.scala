...
object iOS extends LogSupport {
          ...
          Parser("apps", "download a given list of apps")
            .addSubparser(
              Parser("manual", "manually specify download parameter")
                ...
                .addPositional(
                  "list",
                  "if a file then it is assume dto be a chart list otherwise it is assume dto be a csv"
                )
                .addDefault[(ParsingResult, Config) => Unit](
                  "func",
                  downloadAppsManual
  ...
  private def downloadChartListMain(
    ...
    val (appLists, report): (Map[String, MobileAppList], ActionReport) =
      AppListAction.download(IOS, conf.ios.categories, listsFolder)
  ...
  private def downloadAppsAutomatedMain(
    ...
    val allApps =
      currentAppChartList.flatMap(_._2.apps).toList
