...
object Apmi extends LogSupport {
  ...
  private def ensureFolderStructureContinue(
    ...
    folderStructure match {
      ...
      case None =>
        assert(
          downloadFolderRoot.charAt(downloadFolderRoot.length - 1) == '/',
  ...
  @nowarn
  private def getFinishedElements(
      folder: String,
      suffix: String
  ): Set[String] = {
    fsi
