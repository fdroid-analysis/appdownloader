...
object Android extends LogSupport {
  ...
  val parser: Parser = Parser("android", "perform actions for android")
    .addSubparser(
      Parser("download", "download app lists, privacy labels, and ipas")
        .addFlag(
          "continue",
          "c",
          "continue",
        ...
        .addSubparser(
          Parser("chart-list", "download the current chart lists only")
            .addDefault[(ParsingResult, Config) => Unit](
              "func",
              this.downloadChartListMain
            )
        )
        .addSubparser(
          Parser("apps", "download a given list of apps")
            .addSubparser(
              Parser("manual", "manually specify download parameter")
                .addPositional("folder", "the folder into which to download")
                .addPositional(
                  "list",
                  "if a file then it is assumed to be a chart list otherwise it is assumed to be a csv"
                )
                .addDefault[(ParsingResult, Config) => Unit](
                  "func",
                  this.downloadAppsManual
                )
            )
            .addSubparser(
              Parser("manual-via-txt", "manually specify download parameter")
                .addPositional("folder", "the folder into which to download")
                .addPositional("list", "a txt file containing app ids")
                .addDefault[(ParsingResult, Config) => Unit](
                  "func",
                  this.downloadAppsManualAppIds
                )
            )
            .addSubparser(
              Parser(
                "automated",
                "leverage the config for downloading of the apps"
              )
                .addDefault[(ParsingResult, Config) => Unit](
                  "func",
          ...
          Parser("labels", "download a given list of labels")
            .addSubparser(
              Parser("manual", "manually specify download parameter")
                ...
                .addDefault[(ParsingResult, Config) => Unit](
                  "func",
                  this.downloadLabelsManualMain
                )
            )
            .addSubparser(
              Parser(
                "automated",
                "leverage the config for downloading of the labels"
              )
                .addDefault[(ParsingResult, Config) => Unit](
                  "func",
                  this.downloadLabelsAutomatedMain
                )
            )
        )
        .addSubparser(
          Parser(
            "full-chain",
            "do everything from getting the current top list, downloading labels, downloading apps"
          )
            .addDefault[(ParsingResult, Config) => Unit](
              "func",
              this.downloadTopAppsMain
            )
        )
    )

  private def downloadAppsManual(
      pargs: ParsingResult,
      conf: Config
  ): ActionReport = {
    ...
    val ids: List[String] = getTargetAppIdsFromPargs(
      pargs.getValue[String]("list")
    )
    val apps: List[MobileApp] = ids.map { id => MobileApp("", id, "", 0, "") }
    val report = AppDownloadAction.download(apps, downloadFolder, conf, ANDROID)
    report.fails.foreach { case (app, err) =>
      error(s"$app : $err")
    }
    report
  }

  private def downloadAppsManualAppIds(
      pargs: ParsingResult,
      conf: Config
  ): ActionReport = {
    AppDownloader.readinessCheckGooglePlayTool(conf.android.googleplay)
    ...
    val apps: List[MobileApp] = fsi
      ...
      .split("\n")
      .map(_.stripSuffix(".apk"))
      .map {id => MobileApp("", id, "", 0, "")}
      .toList
    val report = AppDownloadAction.download(apps, downloadFolder, conf, ANDROID)
    report.fails.foreach { case (app, err) =>
      error(s"$app : $err")
  ...
  private def downloadLabelsManualMain(
  ...
  private def downloadTopAppsMain(pargs: ParsingResult, conf: Config): Unit = {
    try {
      ...
      val (baseFolder, _, _, _) = ensureFolderStructure(
        conf.downloadFolderRoot + conf.android.osFolderName + "/",
        pargs.getValue[Boolean]("continue")
      )
      val chartReport: ActionReport = this.downloadChartListMain(pargs, conf)
      val labelsReport: ActionReport =
        this.downloadLabelsAutomatedMain(pargs, conf)
  ...
  /** download the current chart lists and store them in the configured folder
    ...
    */
  private def downloadChartListMain(
      pargs: ParsingResult,
      conf: Config
  ): ActionReport = {
    ...
    val (appLists, report): (Map[String, MobileAppList], ActionReport) =
      AppListAction.download(ANDROID, conf.android.categories, listsFolder)
  ...
  private def downloadAppsAutomatedMain(
      pargs: ParsingResult,
      conf: Config
  ): ActionReport = {
    AppDownloader.readinessCheckGooglePlayTool(conf.android.googleplay)
    ...
    AppDownloadAction.download(
      allApps.slice(0, List(allApps.length, conf.maxAmountApps).min),
      appsFolder,
      conf,
      ANDROID
    )
  ...
  private def downloadLabelsAutomatedMain(
      @nowarn pargs: ParsingResult,
      @nowarn conf: Config
  ): ActionReport = {
    val (_, _, labelsFolder, _) = ensureFolderStructure(
      conf.downloadFolderRoot + conf.android.osFolderName + "/",
      pargs.getValue[Boolean]("continue")
    )
    info("starting automated label acquisition")
    val charts = conf.android.categories.map(_.name)
    val currentAppChartList = getCurrentAppChartState(
      charts,
      conf.downloadFolderRoot + conf.android.osFolderName + "/"
    )
    val allAppIds =
      currentAppChartList.flatMap(_._2.apps.map(_.bundleId)).toList
